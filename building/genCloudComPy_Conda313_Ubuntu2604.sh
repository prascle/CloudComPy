...
export CLOUDCOMPY_BUILD=${HOME}/projets/CloudComPy/buildConda313                       # CloudComPy build directory
export CLOUDCOMPY_INSTDIR=${HOME}/projets/CloudComPy/installConda                      # directory for CloudComPy installs
export CLOUDCOMPY_INSTNAME=CloudComPy313                                               # CloudComPy install directory name
export CLOUDCOMPY_INSTALL=${CLOUDCOMPY_INSTDIR}/${CLOUDCOMPY_INSTNAME}                 # CloudComPy install directory
export CLOUDCOMPY_TARFILE=CloudComPy_Conda313_Linux64_"$(date +"%Y%m%d-%H%M")".tgz     # CloudComPy Binary tarfile (will be in ${CLOUDCOMPY_INSTDIR}
...
export CONDA_ENV=CloudComPy313                                                         # conda environment name
...
cloudcompy_setenv()
{
    ...
    conda list > ${CLOUDCOMPY_SRC}/building/conda-list_313_Ubuntu26.04 || error_exit "access problem to ${CLOUDCOMPY_SRC}"
...
cloudcompy_configure()
{
    ...
    -DFBX_SDK_LIBRARY_FILE:FILEPATH="${FBXSDK_REP}/lib/release/libfbxsdk.a" \
    ...
    -DPYTHON_PREFERED_VERSION:STRING="3.13" \
...
cloudcompy_build()
{
    ...
    # libTKHLR.so libTKBO.so libTKPrim.so \
